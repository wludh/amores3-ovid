...
assert 'indented_lines' in metadata, 'Record observed full-page line insets explicitly before building a new candidate'
indented=set(metadata['indented_lines'])
assert indented <= {n for n,_ in lines}, 'Indentation inventory contains an unknown verse'
...
for n,content in lines:
 ...
 for marginal in metadata.get('marginalia',{}).get(str(n),[]):
  E.SubElement(lg,t('fw'),{'type':marginal['type'],'place':marginal['place'],'corresp':f'#O-{poem}-l{n}'}).text=marginal['text']
 if poem=='3.1':
 ...
 if n in indented:attrs['rend']='indent'
 verse=E.SubElement(lg,t('l'),attrs);verse.text=content
 for index,mark in enumerate(metadata.get('marks',{}).get(str(n),[]),1):
  assert mark['position'] in ('before','after')
  element=E.Element(t('metamark'),{X:f'O-{poem}-mark-{n}-{index}','function':'unspecified','place':mark['place']});element.text=mark['text']
  if mark['position']=='before':
   element.tail=' '+(verse.text or '');verse.text=None;verse.insert(0,element)
  else:verse.append(element)
...
def indent_structure(element,depth=0):
 if (element.text or '').strip() or any((child.tail or '').strip() for child in element):return
 if len(element):
  element.text='\n'+'  '*(depth+1)
  for child in element:
   indent_structure(child,depth+1);child.tail='\n'+'  '*(depth+1)
  element[-1].tail='\n'+'  '*depth
indent_structure(r);out.write_text('<?xml version="1.0" encoding="UTF-8"?>\n'+E.tostring(r,encoding='unicode')+'\n');print(out)
